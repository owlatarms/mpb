...
bool Wait(uint32_t *mark, uint32_t wait_ticks)
{
  ...
  if (*mark == 0) {
    ...
    // Метка хранит время начала ожидания
    *mark = system_ticks;
  ...
  // Если разность системных квантов и метки оказывается
  // больше времени ожидания, то выполняется сброс этой метки
  // (т.е. интервал ожидания истёк)
  if (system_ticks - *mark >= wait_ticks) {
    *mark = 0;
  }
