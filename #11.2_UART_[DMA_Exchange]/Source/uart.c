...
volatile uint8_t UART_RX_BUF[UART_RX_BUF_SIZE + 1] = {0};
volatile uint8_t UART_TX_BUF[UART_TX_BUF_SIZE] = {0};

// Флаги завершения циклов обмена
volatile bool flg_uart_rx_upd = false;  // Обновление буфера приёмника
volatile bool flg_uart_tx_eoc = false;  // Завершения цикла передач
...
void UART_Init(void)
{
  ...
  MDR_UART2->LCR_H = (1 << UART_LCR_H_PEN_Pos)   // Контрольный бит (включён)
                   ...
                   | (0 << UART_LCR_H_FEN_Pos)   // Буфер FIFO (не используется)
  ...
  MDR_UART2->IMSC = UART_IMSC_RXIM;  // Прерывание при приёме
  ...
  MDR_UART2->DMACR |= UART_DMACR_RXDMAE;  // Запрос при приёме данных
...
uart_status_t UART_Transmit(volatile uint8_t *data, size_t size)
{
  ...
  static uart_fsm_t fsm_state = UART_FSM_RDY;

  // Состояние «Интерфейс готов»
  if (fsm_state == UART_FSM_RDY) {

    // Конфигурация DMA
    // Адрес источника данных
    DMA_CONTROL_DATA[2].DMA_SRC_DATA_END_PTR = (uint32_t)&data[size - 1];

    // Кол-во передач
    DMA_CONTROL_DATA[2].DMA_CHANNEL_CFG |= ((size - 1) << DMA_CHANNEL_CFG_N_MINUS_1_Pos);

    // Запрос от UART к DMA на передачу
    MDR_UART2->DMACR |= UART_DMACR_TXDMAE;

    // Переход к состоянию «Интерфейс занят»
    fsm_state = UART_FSM_BSY;

    // Выход с флагом «Приём инициирован»
    return UART_TRX_INITIATED;
  }

  // Состояние «Интерфейс занят»
  if (fsm_state == UART_FSM_BSY) {

    // Если цикл передач завершился...
    if (flg_uart_tx_eoc == true) {

      // Сброс флага
      flg_uart_tx_eoc = false;

      // Переход к состоянию «Интерфейс готов»
      fsm_state = UART_FSM_RDY;

      // Выход с флагом «Приём завершён»
      return UART_TRX_COMPLETED;
    }
  }

  // Выход с флагом «Приём в процессе»
  return UART_TRX_INPROCESS;
}
...
void UART2_IRQHandler(void)
{
  ...
  // Установка флага обновления буфера приёмника
  flg_uart_rx_upd = true;
}
